...
<div class="block blogModule boxPlain">
   ...
   <div class="block_content sdsbox-content">
      <ul class="popularArticles">
		...
		src="{$modules_dir}/smartblog/images/{$post.post_img}-home-small.jpg" style="margin-right:5px;">
				</a>
		<a class="title"  title="{$post.meta_title}" href="{smartblog::GetSmartBlogLink('smartblog_post',$options)}">{$post.meta_title}</a><br />
	      <span class="info">{$post.created|date_format:"%d-%m-%Y"}</span>
	    </li> 
	{/foreach}
      </ul>
   </div>
